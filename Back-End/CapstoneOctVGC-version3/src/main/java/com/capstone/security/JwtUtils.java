 ...
 * authentication.getPrincipal(); return Jwts.builder()
 * .setSubject(userPrincipal.getEmail()) .setIssuedAt(new Date())
 * .setExpiration(new Date(new Date().getTime()+jwtExpirationMs))
 * .signWith(SignatureAlgorithm.HS512, jwtSecret) .compact(); }
 * 
 * public String getEmailFromJwtToken(String token) { return
